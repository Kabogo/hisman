...
function sendInquiry(e){e.preventDefault();const fields=[...e.target.elements].filter(el=>el.value).map(el=>el.value).join('%0A');location.href=`mailto:info@hismanproperty.com?subject=Hisman Property Consultants LTD Inquiry&body=${fields}`}
render();
